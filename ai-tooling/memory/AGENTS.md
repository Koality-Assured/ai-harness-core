...
The isolation + dispatch orchestration operational memory lives under [`agent/router/`](./agent/router/) (`skill-agent-dispatch.md`). Router owns spawn/isolation/dispatch orchestration and writes session-end checkpoints here. `ai-tooling-ops` owns skill/agent body authoring; spawn it for memory-* only when that skill is the material user task.
