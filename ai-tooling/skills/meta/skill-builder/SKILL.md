...
contracts:
  inputs:
  - Target skill name, family directory, owner agent ID, isolation mode, description
  outputs:
...
2. Choose family subfolder (`google/`, `aws/`, `azure/`, `gcp/`, `memory/`, `cost-layers/`, `git/`, `reporting/`, `admin/`, `meta/`), `name`, `owner_agent`, `rank`, `isolation`.
...
4. Write `ai-tooling/skills/<family>/<name>/SKILL.md` with required Schema V2 frontmatter and the nine `##` headings. Link; do not paste Critical rules. `## How to use` must discover via `qmd search` (no tree walks). `## Security` must include the sentence starting `Inherits Critical cost layers` and name qmd, ast-grep, and Headroom.
