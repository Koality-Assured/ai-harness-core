...
High when invoked: use primary sources; do not invent framework subcategory IDs.

## Source of truth

- In-repo corpus first via `qmd search` / `qmd get` on kebab-case topic pages under `docs/` and `references/` (not README)
- Vendor/framework primary sources on the internet
...
7. After drafting the research note, apply [`anti-slop`](../anti-slop/SKILL.md) then [`humanizer`](../humanizer/SKILL.md) in this session — do not re-spawn artifact-agent for a quality pass on your own draft. Skip out-of-scope surfaces (code, logs, schemas).
